...
class AstroImuPublisher :
	def __init__(self):
		...
		# Check if minified version
		self.minified = rospy.get_param("~minified", False)

		if self.minified:
			rospy.loginfo("Astro IMU Publisher (Minified)")
		else :
	...
	def raw_data_callback(self, msg):
		...
		if msg.data[0] == msg.data[1] == msg.data[2] == msg.data[3] == 0:
			rospy.logwarn("Bad quaternion. Setting to 0 0 0 1")
		else :
			w = msg.data[0]
		x = msg.data[1]
		y = msg.data[2]
		...
		# Linear acceleration
		self.imu_data.linear_acceleration.x = msg.data[4]
		self.imu_data.linear_acceleration.y = msg.data[5]
		self.imu_data.linear_acceleration.z = msg.data[6]
		# Angular velocity
		self.imu_data.angular_velocity.x = msg.data[7]
		self.imu_data.angular_velocity.y = msg.data[8]
		self.imu_data.angular_velocity.z = msg.data[9]
		# Temperature
		self.imu_temp.temperature = msg.data[10]

		if not self.minified:
			# Euler (convert to radians)
			self.imu_pose.pose.pose.orientation = self.imu_data.orientation
			self.imu_pose.pose.pose.position.x = msg.data[11]*pi/180 # roll
			self.imu_pose.pose.pose.position.y = msg.data[12]*pi/180 # pitch
			self.imu_pose.pose.pose.position.z = msg.data[13]*pi/180 # yaw
			# Magnetic field (convert from uT to T)
			self.imu_mag.magnetic_field.x = msg.data[14]*1.0e-6
			self.imu_mag.magnetic_field.y = msg.data[15]*1.0e-6
			self.imu_mag.magnetic_field.z = msg.data[16]*1.0e-6		
			# System calibration status
			self.imu_status.status[0].level = int(msg.data[17])
			# Accel calibration status
			self.imu_status.status[1].level = int(msg.data[18])
			# Gyro calibration status
			self.imu_status.status[2].level = int(msg.data[19])
			# Mag calibration status
			self.imu_status.status[3].level = int(msg.data[20])
